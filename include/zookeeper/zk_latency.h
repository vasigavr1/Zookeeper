...
static inline void
latency_counters_init(latency_counters_t* lt_cnt)
{
    lt_cnt->max_latency;
    lt_cnt->total_measurements;
    for(int i = 0; i < ZK_LATENCY_BUCKETS + 1; i++){
       lt_cnt->histogram[i] = 0;
    }
}

static inline void
bkkeep_latency(latency_counters_t* lt_cnt, int useconds)
{
    lt_cnt->total_measurements++;

    if (useconds > ZK_MAX_LATENCY){
        lt_cnt->histogram[ZK_LATENCY_BUCKETS]++;
    }else {
        lt_cnt->histogram[useconds / ZK_LATENCY_PRECISION]++;
    }

    if(lt_cnt->max_latency < useconds){
        lt_cnt->max_latency = useconds;
    }
}
...
static inline void
stop_latency_measurement(latency_counters_t* lt_cnt, struct timespec *start)
{
    struct timespec end;
...
//	printf("Latency of Read %u us\n", useconds);
    bkkeep_latency(lt_cnt, useconds);
}

static inline void
dump_latency_stats2file(latency_counters_t* lt_cnt, char *filename, char* prefix)
{
    FILE *latency_stats_fd = fopen(filename, "w");
    fprintf(latency_stats_fd, "#---------------- %s Latency --------------\n", prefix);
    for(int i = 0; i < ZK_LATENCY_BUCKETS; ++i)
        fprintf(latency_stats_fd, "%s: %d, %ld\n", prefix, i * ZK_LATENCY_PRECISION, lt_cnt->histogram[i]);
    fprintf(latency_stats_fd, "%s: -1, %ld\n", prefix, lt_cnt->histogram[ZK_LATENCY_BUCKETS]); //print outliers
    fprintf(latency_stats_fd, "%s-hl: %d\n", prefix, lt_cnt->max_latency); //print max read latency
